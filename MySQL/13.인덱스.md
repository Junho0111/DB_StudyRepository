...
### 7. 시나리오별 사용 예시 (Scenario & Example)

* **카디널리티 기반 선택**: "회원 테이블에서 검색 효율이 가장 높은 컬럼에 인덱스 생성"
```sql
-- 중복도가 높은 '성별'보다는 중복이 거의 없는 '이메일'에 인덱스를 거는 것이 훨씬 유리함
CREATE INDEX idx_member_email ON member(email);

```
<br>

* **자주 사용되는 조회 조건 최적화**: "상품명으로 검색하는 기능이 빈번할 때"
```sql
-- WHERE 절에 자주 쓰이는 name 컬럼에 인덱스를 생성하여 전체 테이블 스캔(Full Scan) 방지
SELECT * FROM product WHERE name = '맥북 프로';
CREATE INDEX idx_product_name ON product(name);

```
<br>

* **복합 인덱스(Composite Index) 활용**: "카테고리 내에서 가격순으로 조회하는 빈도가 높을 때"
```sql
-- 컬럼 순서가 중요함. (카테고리, 가격) 순으로 생성 시 카테고리 필터링과 가격 정렬을 동시에 최적화함
CREATE INDEX idx_category_price ON product(category, price);

-- 인덱스 활용 가능 예시
SELECT * FROM product WHERE category = '가전' ORDER BY price DESC;

```
<br>

* **실행 계획(EXPLAIN) 확인**: "내가 만든 인덱스가 실제로 쿼리에서 사용되는지 검증"
```sql
-- 쿼리 앞에 EXPLAIN을 붙여 'key' 항목에 생성한 인덱스명이 나오는지 확인함
EXPLAIN SELECT * FROM product WHERE name = '아이폰';
