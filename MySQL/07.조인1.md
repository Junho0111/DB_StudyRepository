...
* **기본 내부 조인**: "주문 내역과 해당 주문을 한 고객의 이름을 함께 조회"
```sql
-- orders와 member 테이블에서 member_id가 일치하는 행만 결합함
SELECT o.order_id, m.name, o.order_date
FROM orders o
...
* **3개 이상의 테이블 조인**: "주문 내역, 고객명, 그리고 주문한 상품명을 한꺼번에 조회"
```sql
-- 여러 번의 JOIN을 통해 연쇄적으로 데이터를 연결함
SELECT o.order_id, m.name AS member_name, p.name AS product_name
FROM orders o
...
* **조건 및 정렬 추가**: "서울에 사는 고객의 주문 내역만 최신순으로 조회"
```sql
SELECT o.order_id, m.name, m.address, o.order_date
FROM orders o
JOIN member m ON o.member_id = m.id
...
* **조인 후 집계**: "고객별 총 주문 금액을 계산하여 출력"
```sql
SELECT m.name, SUM(p.price) AS total_payment
FROM orders o
JOIN member m ON o.member_id = m.id
JOIN product p ON o.product_id = p.id
GROUP BY m.name;
