...
* **기본 키 및 필수 값 설정 (PK, NOT NULL)**: "회원 가입 시 반드시 필요한 정보 강제"
```sql
-- id는 자동 증가하는 PK로, 이름과 이메일은 빈 값을 허용하지 않음
CREATE TABLE member (
...
* **참조 무결성 유지 (FOREIGN KEY)**: "존재하지 않는 상품의 주문을 원천 차단"
```sql
-- orders 테이블의 product_id는 반드시 product 테이블의 id에 존재해야 함
ALTER TABLE orders 
...
* **기본값 설정 (DEFAULT)**: "데이터 입력 시 누락된 항목에 자동 값 부여"
```sql
-- 가입일시를 입력하지 않아도 현재 시간이 자동으로 기록됨
ALTER TABLE member 
MODIFY join_date DATETIME DEFAULT NOW();

-- 주문 상태의 기본값을 'READY'로 설정함
ALTER TABLE orders 
MODIFY status VARCHAR(20) DEFAULT 'READY';

```
<br>

* **데이터 삭제 시 연쇄 동작 (ON DELETE)**: "부모 데이터 삭제 시 자식 데이터 처리 정책"
```sql
-- 회원이 탈퇴(삭제)되면 해당 회원의 주문 내역도 자동으로 삭제함 (CASCADE)
ALTER TABLE orders 
