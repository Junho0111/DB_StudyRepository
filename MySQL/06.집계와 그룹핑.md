...
* **기본 그룹화 및 집계**: "카테고리별 상품 개수와 평균 가격 조회"
```sql
-- category로 그룹을 묶고 각 그룹의 개수와 평균을 계산함
...
* **WHERE와 HAVING의 조합**: "가격 1만 원 이상인 상품들 중, 총 재고가 50개 초과인 카테고리만 추출"
```sql
-- 1. WHERE: 개별 상품 가격으로 먼저 필터링
-- 2. HAVING: 그룹화된 후의 재고 합계로 다시 필터링
...
* **다중 컬럼 그룹화**: "고객별로 어떤 카테고리의 상품을 몇 번 주문했는지 집계"
```sql
-- 고객명과 카테고리 두 가지를 기준으로 세분화하여 그룹화함
SELECT member_name, category, COUNT(*) AS order_cnt
FROM orders
GROUP BY member_name, category
ORDER BY member_name ASC, order_cnt DESC;

```
<br>

* **중복 제거 후 집계 (DISTINCT)**: "우리 쇼핑몰에서 상품을 구매한 이력이 있는 순수 고객 수 계산"
```sql
-- 한 고객이 여러 번 주문해도 1명으로 카운트함
SELECT COUNT(DISTINCT member_id) AS unique_customer_count
FROM orders;
...
* **GROUP BY의 제약**: `GROUP BY`를 사용할 때 `SELECT` 절에는 그룹화의 기준이 된 컬럼이나 집계 함수(`SUM`, `COUNT` 등)만 올 수 있음. 기준 외의 일반 컬럼을 넣으면 부정확한 데이터가 조회될 수 있음.
