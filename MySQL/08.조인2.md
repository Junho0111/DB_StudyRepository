...
* **LEFT JOIN (전체 목록 유지)**: "주문 기록이 있든 없든 모든 회원과 그들의 주문 일자를 조회"
```sql
-- 주문을 한 번도 안 한 회원은 order_date가 NULL로 표시됨
...
* **데이터 공백 찾기 (IS NULL)**: "아직 한 번도 판매되지 않은 상품 목록 추출"
```sql
-- 상품 테이블을 기준으로 주문 테이블을 붙인 뒤, 주문 정보가 없는 행만 필터링
SELECT p.name
FROM product p
...
* **셀프 조인 (Self Join)**: "직원 테이블에서 각 직원의 매니저 이름을 함께 표시"
```sql
-- 동일한 employee 테이블에 별칭(e, m)을 주어 상급자 정보를 가져옴
...
* **다중 외부 조인**: "모든 회원에 대해 주문 내역과 주문한 상품의 이름까지 조회"
```sql
-- 여러 테이블을 LEFT JOIN으로 이어가며 기준 테이블(member)의 데이터를 보존함
SELECT m.name, p.name AS product_name, o.order_date
FROM member m
LEFT JOIN orders o ON m.id = o.member_id
