...
* **복잡한 조인 단순화**: "매번 작성하기 번거로운 주문-회원-상품 통합 정보 뷰 생성"
```sql
-- 복잡한 조인 쿼리를 v_order_summary라는 이름의 가상 테이블로 정의함
CREATE VIEW v_order_summary AS
SELECT o.id AS order_id, m.name AS member_name, p.name AS product_name, o.order_date
FROM orders o
JOIN member m ON o.member_id = m.id
JOIN product p ON o.product_id = p.id;

-- 호출 시에는 일반 테이블처럼 간결하게 사용함
SELECT * FROM v_order_summary WHERE member_name = '김영한';

```
<br>

* **보안 강화 (데이터 필터링)**: "민감한 개인정보를 제외하고 업무에 필요한 컬럼만 노출"
```sql
-- 주민번호, 비밀번호 등 민감 정보를 제외한 회원 공개용 뷰 생성
...
* **조건부 데이터 제공**: "자주 조회하는 '배송 중' 상태의 데이터만 따로 모아 뷰로 관리"
```sql
CREATE VIEW v_shipping_now AS
SELECT * FROM orders
...
* **통계 쿼리 캡슐화**: "카테고리별 매출 통계를 뷰로 만들어 보고서 쿼리 단순화"
```sql
